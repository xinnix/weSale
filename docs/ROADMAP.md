...
| **1.8 KF 逼单真实化**  | ✅ 已完成                  | 2026-09-09 核对：真实建单 + 小程序卡片已上线（c5c6891），详见已完成表                                                                                                                                                               |
| **1.5 mall REST**      | ✅ 后端完成                | 2026-09-09 落地，详见已完成表；小程序端 `src/api/mall.ts` 封装随 1.7 一起做                                                                                                                                                         |
| **1.2 DI 修复**        | ✅ 已完成                  | 2026-09-09 落地，详见已完成表；回调可观测性同步核对通过（双层日志已有）                                                                                                                                                             |
| 1.7 小程序商城页面     | 未开始（**当前最高优先**） | mall 首页/商品详情/订单确认（含地址）/支付结果/订单列表/订单管理页 + 地址管理 + `src/api/mall.ts`；**同时清理 11 个死页面注册**（当前 miniapp build 失败的根因）；KF 卡片落地路径 `pages/order/confirm/index?orderNo=` 必须在此实现 |
...
> 排期逻辑（2026-09-09 更新）：1.5（后端）/1.2 已完成；下一步 **1.7 小程序商城页面**（Phase 1 收尾主链，含 KF 卡片落地页）；随后 1.9/1.10/1.1 商家化收尾。Phase 1 完成后立即启动 Phase 2 M1（侧边栏壳 + F2/F8 引入层）。PRD v2.0 已定稿（2026-09-09），开放问题拍板结果见 `docs/prd.md` 第九节。
