...
> 最后更新：2026-09-16。本文档是项目路线图与进度台账。当前方向：**B2B 商家工具 · 人机协同形态**（微信客服 AI 全自动接待 + 企微侧边栏 AI Copilot + 会员积分留存），2026-09-08 由「全自动销售」修订而来，决策依据见文末附注，产品定义见 `docs/prd.md`（PRD v2.0，SSOT），演示链路对齐见 `docs/demo-blueprint.md`。
...
Phase 2    企微侧边栏 AI Copilot（壳→AI生成→知识库RAG→动态卡片）     🔨 M1 真联调+M2 真机验证完成；M4-lite 已实现；余 M3/M4
...
### ✅ M1 已落地（2026-09-13 代码侧；09-16 真联调打通）
...
> ✅ **真联调已完成（09-15/16）**：OAuth、白名单、@wecom/jssdk 双签名鉴权、归因（真实好友 ID 验证 Contact 自动建立）均已实测通过。客户联系事件已做 KfController 兜底分发；sendChatMessage 已在当前客户端验证，PC/移动/Mac 三端差异维持备注。

### ✅ M2 已落地（2026-09-14 代码侧；09-16 真机验证通过）
...
### ✅ 真联调打通 + M4-lite + 粘贴对话资产化（2026-09-15/16，企微真实环境全链路验证）

| 任务                           | 内容                                                                                                                                                                                                                                                                                              | 关键产物                                |
| ------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------- |
| 同域托管                       | 后端 NestJS 托管 `apps/sidebar/dist` 于 `/sidebar/`（复用 frp 隧道）；index.html 禁缓存                                                                                                                                                                                                           | `main.ts`                               |
| JS-SDK 修复链（真联调 5 连修） | 全局对象 `wx`（非 ww）；`wx.config` 字段 `appId`（非 corpid，否则企业上下文未绑定）；agentConfig 签名需 **agent_config ticket**（`type=agent_config`，与企业 jsapi_ticket 不可混用）；**http 未解包 envelope 致 corpid 恒 undefined（根因）**；access_token 缓存 key 加 secret 指纹（多应用串用） | `useWxAgent`/`wecom-api`/`sidebar-auth` |
| SDK 迁移                       | 弃用旧 `wx.config`/`wx.agentConfig` 链路，迁移官方 **@wecom/jssdk `ww.register`**（企业/应用双签名工厂 + onXxx 回调）                                                                                                                                                                             | `useWxAgent`                            |
| 真机全链路验证                 | OAuth→白名单→register 双鉴权→getCurExternalContact→画像（OneID 订单+标签）→3 策略流式→发送→采纳率上报，全部在企微聊天工具栏真实通过                                                                                                                                                               | —                                       |
| 粘贴对话（M2 增强）            | 私聊不开放的合规替代：侧边栏粘贴框→意图/心理/三策略；实测粘贴"易碎"→ SAFETY_CONCERN + 焦虑 0.75 + 三策略围绕破损包赔                                                                                                                                                                              | `CopilotPanel`/analyze                  |
| 粘贴对话资产化                 | `PastedConversation` 表落库 + 意图回填 + **意图自动写 Contact.tags**（身份标签闭环）+ 合并进"最近会话"展示                                                                                                                                                                                        | 迁移 `20260916045048`                   |
| M4-lite 产品卡片               | 在售商品列表 + 封面上传 thumb_media_id（缓存 2.5d）+ `ww.sendChatMessage(miniprogram)` + 上报；跳转商品详情页                                                                                                                                                                                     | `sidebar-products.service`              |
| 归因修复                       | 客户联系事件打到客服 endpoint 被忽略 → KfController 兜底分发；真实好友 ID 验证 Contact 自动建立+关联                                                                                                                                                                                              | `kf.controller`/`spike-attribution`     |
| 遗留待办                       | `.env MINIAPP_APPID` 更新为正式 appid；意图分类偶发空响应（加重试）；mock 数据清理；`LLM_API_KEY` 轮换；客户联系回调 URL 规范化（可选）                                                                                                                                                           | —                                       |
...
- **2026-09-15/16 M1 真联调打通 + M4-lite + 粘贴对话资产化**：企微聊天工具栏真实环境全链路跑通（OAuth→白名单→@wecom/jssdk 双签名鉴权→定位客户→画像→3 策略流式→发送→采纳率上报）。真联调暴露并修复 5 处：`wx` 全局对象（非 ww）、`wx.config` 字段 `appId`（非 corpid）、agentConfig 需 agent_config ticket（与企业 jsapi_ticket 不可混用）、**http 未解包 envelope（corpid 恒 undefined 的根因）**、access_token 缓存串用；最终迁移官方 `@wecom/jssdk`（ww.register）。新增粘贴对话（私聊不开放的合规替代，资产化进最近会话+意图自动打标）与产品卡片选择发送（M4-lite 提前）。归因 endpoint 错位修复（KfController 兜底分发客户联系事件）。
